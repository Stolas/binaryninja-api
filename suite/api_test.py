	...
	def test_printer(self):
		class MyTypePrinter(TypePrinter):
			...
			def get_type_tokens_before_name(self, type: types.Type, platform: Optional[Platform], base_confidence: int,
											parent_pointer, escaping: TokenEscapingType) -> List[
			...
			def get_type_tokens_after_name(self, type: types.Type, platform: Optional[Platform], base_confidence: int,
										   parent_pointer, escaping: TokenEscapingType) -> List[InstructionTextToken]:
				return [
					InstructionTextToken(InstructionTextTokenType.TextToken, " bottom text", 0),
				]
		...
		result = tp.get_type_tokens_before_name(Type.void(), None, 255, False, TokenEscapingType.NoTokenEscapingType)
		assert result is not None
		assert len(result) == 1
		assert result[0].text == "the type is: "
		result = tp.get_type_tokens_after_name(Type.void(), None, 255, False, TokenEscapingType.NoTokenEscapingType)
		assert result is not None
		assert len(result) == 1
		assert result[0].text == " bottom text"
		result = tp.get_type_string(Type.void(), None, QualifiedName(['test']), TokenEscapingType.NoTokenEscapingType)
		assert result is not None
		assert result == "the type is: test bottom text"
		result = tp.get_type_string_before_name(Type.void(), None, TokenEscapingType.NoTokenEscapingType)
		assert result is not None
		assert result == "the type is: "
		result = tp.get_type_string_after_name(Type.void(), None, TokenEscapingType.NoTokenEscapingType)
		assert result is not None
		assert result == " bottom text"
		result = tp.get_type_lines(Type.void(), BinaryView.new(b''), QualifiedName(['test']), 80, False, TokenEscapingType.NoTokenEscapingType)
		assert result is not None
		assert len(result) == 1
