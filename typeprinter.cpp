	...
	bool parentPointer, BNTokenEscapingType escaping)
{
	...
	bool success = BNGetTypePrinterTypeTokensBeforeName(GetObject(), type->GetObject(), platform ? platform->GetObject() : nullptr, baseConfidence, parentPointer, escaping, &tokens, &tokenCount);
	...
	bool parentPointer, BNTokenEscapingType escaping)
{
	...
	bool success = BNGetTypePrinterTypeTokensAfterName(GetObject(), type->GetObject(), platform ? platform->GetObject() : nullptr, baseConfidence, parentPointer, escaping, &tokens, &tokenCount);
